export interface GeoPageSection {
  title: string;
  text: string;
}
...
export interface GeoPageData {
  slug: string;
  title: string;
  category: "Pillars" | "Brands" | "Comparisons" | "Service Guides" | "Locations";
  description: string;
  question: string;
  answer: string;
  keyPoints: string[];
  sections: GeoPageSection[];
  faqs: GeoFaqItem[];
  relatedSlugs: string[];
  comparisonTable?: {
    headers: string[];
 ...
 */
export function getGeoPagesByCategory(category: string): GeoPageData[] {
  return Object.values(GEO_PAGES).filter((page) => page.category.toLowerCase() === category.toLowerCase());
}

/**
 * Returns a specific GEO page by slug
 */
export function getGeoPageBySlug(slug: string): GeoPageData | null {
  return GEO_PAGES[slug] || null;
}
